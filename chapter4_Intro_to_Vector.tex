...
The Real $n$-space $\mathbb{R}^n$ is defined as the set of all possible vectors (or "points") $\vec{v} = (v_1, v_2, v_3, \cdots, v_n)^T$ as defined in Definition \ref{defn:geometvec}, where $v_i$ can take any \textit{real} value, for $i = 1,2,3,\ldots,n$. The member vectors in $\mathbb{R}^n$ are known as $n$-dimensional \textit{real} vectors.
\end{defn}
...
\\
An $n$-dimensional real geometric vectors as in Definition \ref{defn:geometvec} and \ref{defn:real_nspace} can be written as the sum of $n$ \index{Standard Unit Vector}\keywordhl{Standard Unit Vectors} that have a magnitude of $1$, denoted by $\hat{e}_p$, oriented in the positive direction along the $p$-th coordinates axes, $p = 1,2,\ldots,n$. Particularly in the three-dimensional (real) space $\mathbb{R}^3$ where we are living, $\hat{e}_1 = \hat{i} = (1,0,0)^T$, $\hat{e}_2 = \hat{j} = (0,1,0)^T$, $\hat{e}_3 = \hat{k} = (0,0,1)^T$ corresponding to $x$, $y$, $z$ axes respectively.
\begin{defn}
...
\label{defn:dotreal}
The dot product between two $n$-dimensional \textit{real} vectors $\vec{u}$ and $\vec{v}$ are denoted as either $\vec{u} \cdot \vec{v}$, or by matrix notation $\textbf{u}^T\textbf{v}$. They are defined as
\begin{align*}
...
\begin{proper}
\label{proper:dotgeo}
For two vectors $\vec{u}$ and $\vec{v}$, we have
\begin{align*}
\vec{u} \cdot \vec{v} = \norm{\vec{u}}\norm{\vec{v}}\cos\theta
\end{align*}
...
\end{align*}
\end{solution}
From this, the concept of "\index{Orthogonal}\keywordhl{Orthogonal}" becomes an extension of "perpendicular" in higher dimensions. Note that \textit{the zero vector is regarded to be orthogonal to any vector}, so even if $\vec{u}$ or $\vec{v}$ is a zero vector, this properties still hold. \par
...
\label{thm:CauchySch}
Given two vectors $\vec{u}$ and $\vec{v}$ that are $n$-dimensional, the following inequality holds.
\begin{align*}
...
For $\vec{u} = (u_1, u_2, u_3)^T$ and $\vec{v} = (v_1, v_2, v_3)^T$, their cross product $\vec{u} \times \vec{v}$ can be written in the form of a determinant as
\begin{align*}
\vec{u} \times \vec{v} =
\begin{vmatrix}
\hat{i} & \hat{j} & \hat{k} \\
...
\hline
Cross Product, or Vector Product ($\times$) & Two three-dimensional vectors, the order is important (anti-commutative) & Another three-dimensional vector
\\
\hline
\end{tabular}
\end{center}
...
\begin{lstlisting}
# All commands below return zero.
