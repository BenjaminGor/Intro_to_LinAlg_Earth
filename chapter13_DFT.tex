...
\end{bmatrix}
\end{align}
are vectors with $2p+1$ and $N$ entries respectively. The least-square method then sets out to find the best-fit parameters for $\vec{\beta} = (C_0, A_1, B_1, \cdots, A_p, B_p)^T$ to this system of equations. From Theorem \ref{thm:bestfit}, we know that the best parameters are found by
\begin{align}
...
\end{aligned}
\end{align}
Notice that the L.H.S. is a geometric sequence with a common ratio $r = \exp(i 4\pi k/N)$\footnote{which will not be equal to $1$ as we have demanded that $k \leq \frac{N-1}{2}$.}, the sum of which is seen to be
\begin{align}
